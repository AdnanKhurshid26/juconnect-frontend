  ...
  const handleSubmit = async (e) => {
    ...
    if (state.role === "Student") {
      ...
      const response = await fetch(
        appendToUrl(backendUrl, "auth/signup/jums/verify"),
        options
      );
      ...
      const response = await fetch(
        appendToUrl(backendUrl, "auth/signup/faculty/verify"),
        options
      );
  ...
  return (
    <div className="mx-auto max-w-md bg-red-200 p-8 rounded-lg">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className="w-full bg-red-800 text-white p-2 rounded cursor-pointer"
        >
          Submit
        </button>
      </form>
